...
<body>
  <div class="container">
    ...
    ${this.generateUnlinkedTestsSection(report)}
  ...
  private getStyles(): string {
    ...
    * {
    ...
    .unlinked-list li .test-file {
      font-family: monospace;
      color: var(--color-muted);
    }

    .unlinked-list li .test-title {
      font-weight: 500;
    }
  ...
  /**
   * Generate unlinked tests section
   */
  private generateUnlinkedTestsSection(report: CoverageReport): string {
    if (!report.unlinkedTests || report.unlinkedTests.length === 0) return '';

    const items = report.unlinkedTests.map(test => `
      <li>
        <span class="test-title">${this.escapeHtml(test.title)}</span><br>
        <span class="test-file">${test.file}${test.line ? `:${test.line}` : ''}</span>
      </li>`).join('');

    return `
    <section class="card">
      <h2>📋 Unlinked Tests (${report.unlinkedTests.length})</h2>
      <p style="color: var(--color-muted); margin-bottom: 1rem;">
        These tests don't have facet annotations. Consider linking them to facets for better coverage tracking.
      </p>
